...
class Net(nn.Module):
    ...
    def forward(self, x):
        # print(x)
        # print(x.shape) #([batchsize, 12, 8, 7])

        # diversify_data_cpu=self.dict[np.array_str(x.cpu().numpy())]
        # diversify_data=diversify_data_cpu.numpy()
        ...
        flatten_tensor = after_conv.view(x.shape[0], -1)
        #弄完这个得改上面的_batch_norm_fc1
        # print(flatten_tensor.shape)
        fc1_output = self._dropout_fc1(self._prelu_fc1(self._batch_norm_fc1(self._fc1(flatten_tensor))))
        #self._prelu_fc2(self._batch_norm_fc2(self._fc2(fc1_output)))是需要的特征表示

        # fc2_output=self._prelu_fc2(self._batch_norm_fc2(self._fc2(fc1_output)))
