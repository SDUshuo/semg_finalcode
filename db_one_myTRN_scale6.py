...
class Net(nn.Module):
    def __init__(self, number_of_class,num_segments=12, fenlei=True):
        ...
        self.trn = TRNmodule.return_TRN('TRNmultiscale', self.img_feature_dim, 6, number_of_class, self.feature_bottleneck)
        ...
        self.classifier = nn.Sequential(
            nn.Linear(2*self.fushion_2_feature_bottleneck_, self.fushion_2_feature_bottleneck_),
            nn.BatchNorm1d(self.fushion_2_feature_bottleneck_),
            nn.ReLU(),
            nn.Dropout(0.5),
    ...
    def forward(self, x):
        ...
        #将12帧分为4组，每组3帧
        all_fenzu_data = [all_x_flattend[i:i + 6] for i in range(0, len(all_x_flattend), 6)]
        fenzu_trn_data=[]
