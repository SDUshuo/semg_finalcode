...
class Net(nn.Module):
    def __init__(self, number_of_class,num_segments=12, fenlei=True):
        ...
        self.img_feature_dim=56
        self.num_segments=num_segments #有多少帧
        #更改这个隐藏层维度，是TRN最后输出的维度
        self.feature_bottleneck = 100
    ...
    def forward(self, x):
        ...
        for i in range(self.num_segments):
            ...
            conv_feature=self.all_first_conv(x[:, i, :, :].unsqueeze(1)).cuda().view(x.shape[0], -1)#torch.Size([batchsize, 40])
