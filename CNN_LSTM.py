...
class ConvLSTM(nn.Module):
    def __init__(self,in_channels,out_channels,num_class,fenlei=True):
        ...
        self.fenlei =fenlei
        # LSTM layer
        self.lstm = nn.LSTM(input_size=out_channels, hidden_size=out_channels, num_layers=1, batch_first=True)
    ...
    def forward(self, x):
        ...
        # Pass the sequence of feature vectors through LSTM
        out, _ = self.lstm(x)

        # Only take the output from the final timestep
        out = out[:, -1, :]
        if self.fenlei:
            # Pass through a Fully Connected layer for final classification
...
# 4-82.651  3-
    ...
    setup_seed(seed)
    model = ConvLSTM(in_channels=10,out_channels=64).cuda()
    # model = svm.SVC()
    for dataset_index in range(1, 28):
        ...
        for k, data_test_0 in enumerate(test_0_loader, 0):
            ...
            outputs_test_0 = multi_model(concat_input_TCN)
...
def train_model(model, criterion, optimizer, scheduler, dataloaders, people, num_epochs=epochs, precision=1e-8,
    ...
    if not justtest:
            ...
            for phase in ['train', 'val']:
                ...
                if phase == 'val':
                    ...
                    if epoch_loss + precision < best_loss:  # 当前验证损失值加上一个小的精度值小于最佳损失值best_loss
                        ...
                        torch.save(model.state_dict(), 'best_weights_source_wavelet_db1_CNNLSTM.pt')
        ...
        model_weights = torch.load('best_weights_source_wavelet_db1_CNNLSTM.pt')
        model.load_state_dict(model_weights)
        model.eval()
        return model
    else:
