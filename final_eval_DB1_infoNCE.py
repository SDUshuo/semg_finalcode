...
import db_one_model_infoNCE as db_one_model
...
def infoNCE_loss(features1, features2, temperature=0.5):
    ...
    # 计算InfoNCE损失
    loss = -torch.log(torch.diag(similarity_matrix) / torch.exp(similarity_matrix).sum(dim=1))
    loss = loss.mean()
...
def calculate_fitness(seedlist):
    ...
    for dataset_index in range(1, 2):
        ...
        directory = 'saved_data/DB1/52_5/52_5_exercise1_jitr_norm/subject_'+ str(dataset_index) + '/train'
        ...
        # if M  != 12:
        #     # 随机选择 0 到 29 之间的 12 个唯一索引
        #     indices = np.random.choice(M, 12, replace=False)
        #
        #     # 使用这些索引从第二个维度选择数据
        #     X_fine_tune_train = X_fine_tune_train[:, indices, :, :]

        # 准备测试集0的数据
        # directory = 'saved_data/DB1/52_5/subject_' + str(dataset_index) + '/test'
        directory =  'saved_data/DB1/52_5/52_5_exercise1_jitr_norm/subject_'+ str(dataset_index) + '/test'
        ...
        for k, data_test_0 in enumerate(test_0_loader, 0):
            ...
            a,b,outputs_test_0 = multi_model(concat_input_trn,concat_input_TCN)
...
def combined_loss(features1, features2, labels, temperature=0.08):
    # 计算InfoNCE loss
    batch_size = features1.shape[0]
    features = torch.cat([features1, features2], dim=0)
    features = F.normalize(features, p=2, dim=1)
    similarity_matrix = torch.matmul(features, features.T)
    mask = torch.roll(torch.eye(2 * batch_size), batch_size, dims=0).cuda()
    positive_pairs = torch.masked_select(similarity_matrix, mask.bool()).view(batch_size, 2)
    negative_pairs = torch.masked_select(similarity_matrix, ~mask.bool()).view(batch_size, -1)
...
def train_model(model,criterion, optimizer, scheduler,dataloaders, num_epochs=100, precision=1e-8,justtest=False):
    since = time.time()
    best_loss = float('inf')
        ...
        for epoch in range(num_epochs):
            ...
            for phase in ['train', 'val']:
                ...
                for i, data in enumerate(dataloaders[phase], 0):
                    ...
                    optimizer.zero_grad()
                    if phase == 'train':
                        tcn_output, trn_output, outputs = model(inputs, TCN_input)
                        _, predictions = torch.max(outputs.data, 1)

                        ce_loss = criterion(outputs, labels)
                        contrastive_loss = combined_loss(tcn_output, trn_output, labels)
                        # print('contrastive_loss:  '+str(contrastive_loss.item()))
                        # 这里你可以调整alpha值来改变两种损失的权重
