...
def calculate_fitness():
    ...
    for dataset_index in range(1, 2):
        ...
        multi_model = train_model(model,criterion, optimizer, scheduler,
                              dataloaders={"train": trainloader}, precision=precision)
...
def train_model(model,criterion, optimizer, scheduler,dataloaders, num_epochs=epochs, precision=1e-8,justtest=False):
    since = time.time()
    best_loss = float('inf')
