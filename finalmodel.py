...
import TRNmodule
from params_contact import feature_bottleneck
...
class Net(nn.Module):
    def __init__(self,tcn_inputs_channal,number_of_classes):
        ...
        #                                 kernel_size=2, dropout=0.3, fenlei=True).cuda()
        # self.Slowfushion=Wavelet_CNN_Source_Network.Net(number_of_class=number_of_classes).cuda()
        self.lstm_out_dim = 52
        self.CNN_LSTM = CNN_LSTM.ConvLSTM(in_channels=tcn_inputs_channal, out_channels=self.lstm_out_dim,num_class=7,
                                          fenlei=False).cuda()
        # 创建LMF多模态融合
        fushion_dim = 80
        rank = 4
        ...
        self.fenlei_feature = 64 + self.lstm_out_dim
    ...
    def forward(self, input_trn,input_tcn):
        ...
        # slow_optput =self.Slowfushion(input_trn)
        return lmf_output
        # trn_output = self.TRN(input_trn)
        #
        # lstm_output = self.CNN_LSTM(input_tcn)
