...
import copy
"""
该程序文件是一个Python脚本，用于对目标波形进行评估。它导入了numpy、torch和scipy.stats等Python库，以及其他自定义模块。
脚本实现了数据预处理、模型训练和评估等功能。其中，函数calculate_pre_training()用于对模型进行预训练，
pre_train_model()函数用于训练模型，calculate_fitness()函数用于评估模型的性能。在脚本的最后，还有一行代码调用了calculate_fitness()函数进行模型评估。
"""
